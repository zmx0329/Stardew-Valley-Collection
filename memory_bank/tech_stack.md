- 前端：TypeScript + React + Vite；上传后直接调用豆包 Seedream 生图模型按固定提示词生成星露谷风预览；Canvas/SVG 负责识别框与便签叠加，并在模型失败时回退本地像素滤镜占位；Zustand 管状态，上传支持拖拽/拍照。
- 像素滤镜：主要依赖豆包 Seedream 生图输出星露谷像素风；后端使用同一模型结果并用 Pillow 再叠加标签/时间/金币，保证落盘与预览一致；本地 Canvas 滤镜仅作为失败兜底。
...
- 图合成：豆包 Seedream 生图结果 + Pillow 叠加标签/时间/金币，生成最终作品。
